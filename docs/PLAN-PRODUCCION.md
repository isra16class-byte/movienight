...
- [x] **Fix de Google Fonts confirmado en un segundo pase por el navegador**
      (mismo día): con el fix ya aplicado, `document.fonts.status` da
      `loaded` y las 3 tipografías (Monoton/Space Grotesk/VT323) quedan
      aplicadas de verdad en `/`, `/library.html` y `/room/:id` — confirmado
      no solo por ausencia de errores en consola, sino visualmente
      (screenshot de cada página) y por `document.fonts.check(...)`. Sin
      ningún mensaje de CSP/CORS en ninguna de las tres.
