...
// original: el plan proponía `default: null` ("null = nunca expira", presentándolo como un valor ya
// válido hoy) — pero el comportamiento REAL sin nada configurado (ni panel ni .env) siempre fue 24hs
// (ver el fallback hardcodeado en lib/roomStore.js antes de este refactor), nunca "para siempre". Si
// el catálogo hubiera quedado con `default: null`, una instalación nueva sin tocar nada habría pasado
// a tener salas que NUNCA expiran por default — un cambio de comportamiento real, no el refactor
// puramente mecánico que promete la sección 4 del plan. Por eso el `default` de acá es 24 (el mismo
// de siempre), y `nullable: true` deja "nunca expira" disponible como una elección EXPLÍCITA desde el
// panel (guardando el campo vacío), no como lo que pasa sin querer si nadie toca nada.
const SETTINGS = {
  ROOM_TTL_HOURS: { type: 'float', default: 24, min: 0.5, max: 24 * 30, nullable: true },
