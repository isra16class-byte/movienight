...
const { S3Client, ListObjectsV2Command, DeleteObjectCommand, HeadBucketCommand, HeadObjectCommand, ListMultipartUploadsCommand, AbortMultipartUploadCommand, PutObjectCommand, GetObjectCommand } = require('@aws-sdk/client-s3');
...
// --- Conteo de errores para /metrics (Fase 4 del plan de producción) ---------------------------
// Envuelve cualquier operación real contra R2 (subir, presign, listar, borrar, leer cabecera) para
// que un fallo quede contado en `metrics.recordR2Error()` sin tener que repetir un try/catch idéntico
// en cada función de acá abajo. El error original se relanza tal cual — este wrapper solo observa,
// nunca cambia el comportamiento de la función envuelta.
async function withR2ErrorTracking(fn) {
  try {
    return await fn();
  } catch (err) {
    metrics.recordR2Error(err);
    throw err;
  }
}
...
function getClient() {
  ...
  if (!_client) {
    _client = new S3Client({
      ...
      // Fase 2.7 — hallazgo al revisar contra R2 real (2026-09-06): desde @aws-sdk/client-s3
      // v3.729.0 (diciembre 2024), el SDK calcula por default un checksum CRC32 y lo suma como
      // parámetro extra en cualquier PutObject/UploadPart — incluida una URL prefirmada, donde
      // ese checksum queda calculado sobre un body VACÍO (todavía no existe el archivo real al
      // momento de firmar) y viaja horneado en la propia URL como `x-amz-checksum-crc32` /
      // `x-amz-sdk-checksum-algorithm`. R2 no lo implementa y devuelve un 501 `NotImplemented`
      // apenas el navegador intenta subir el archivo real — confirmado como problema conocido y
      // documentado por Cloudflare (ver "AWS SDK v3" en developers.cloudflare.com/r2/examples/aws/
      // aws-sdk-js-v3/) y reportado por varios proyectos (flysystem, vector, LocalStack, etc.) tras
      // esa versión del SDK. `WHEN_REQUIRED` en vez del nuevo default `WHEN_SUPPORTED` restaura el
      // comportamiento de antes de esa versión: solo calcula/valida checksum cuando la operación lo
      // exige de verdad (no es el caso de PutObject), así que no agrega nada al firmar. Aplica a
      // TODAS las funciones de este archivo (suben o no), no solo a getPresignedUploadUrl, porque
      // el cliente se comparte (getClient() es un singleton) — probado localmente comparando la URL
      // firmada con y sin esta opción: sin ella, la URL incluye `x-amz-checksum-crc32`/
      // `x-amz-sdk-checksum-algorithm`; con ella, no aparece ninguno de los dos.
...
async function testConnection() {
  return withR2ErrorTracking(async () => {
...
async function objectExists(key) {
  ...
  try {
    ...
    // Cualquier otro error (credenciales, conexión) sí se propaga, no se confunde con "no existe" —
    // y a diferencia del 404 (un resultado válido), esto sí cuenta como una falla real de R2 para
    // /metrics.
    metrics.recordR2Error(err);
    throw err;
  }
...
// --- Cloudflare R2 — Fase 2.5: leer el rango inicial de un objeto ya subido ---------------------
// Por qué existe esto: en el camino de subida directa por URL prefirmada (Fase 2.7), el archivo va
// del navegador directo al bucket — el server nunca ve esos bytes mientras suben, a diferencia del
// modo streaming (`uploadStream`, arriba) donde sí pasan por acá y se pueden inspeccionar al vuelo.
// Para validar por magic bytes (`lib/fileValidation.js`) un archivo que llegó por ese camino, hace
// falta leerlo DESPUÉS de que ya está en el bucket — pero solo el comienzo, no el objeto entero (que
// puede pesar varios GB): un `GetObjectCommand` con el header `Range` (`bytes=0-N`) le pide a R2/S3
// que devuelva nada más que ese rango, igual que hace un reproductor de video pidiendo de a pedazos.
// Devuelve un Buffer con los bytes leídos (puede ser más chico que `maxBytes` si el objeto entero
// pesa menos que eso).
async function getObjectHead(key, maxBytes) {
  return withR2ErrorTracking(async () => {
    const client = getClient();
...
// --- Cloudflare R2 — Fase 2.4 del plan de producción: orígenes externos para la CSP --------------
// server.js arma la Content-Security-Policy del server (helmet) y, cuando R2 está activo, necesita
// permitir explícitamente dos hosts que si no quedan bloqueados por default (default-src 'self'):
//   - El endpoint de subida prefirmada (getPresignedUploadUrl más arriba) — el navegador hace un PUT
//     directo ahí, necesita entrar en `connect-src`. El SDK de AWS arma esa URL en estilo
//     "virtual-hosted" (el bucket como subdominio, no como parte del path) — confirmado imprimiendo
//     una URL firmada real, no es un dato de la documentación de Cloudflare que pueda cambiar sin
//     que lo notemos acá.
//   - R2_PUBLIC_URL (getPublicUrl más arriba) — de ahí sale el `<video src>`/`<track src>` que ve
//     cada espectador, necesita entrar en `media-src`; y como la biblioteca a veces valida esos
//     archivos con `fetch`/`XHR` además de solo reproducirlos, se agrega también a `connect-src`.
// Vive acá (no en server.js) para que un solo lugar sepa la forma real de esas URLs — la misma razón
// por la que getPublicUrl()/getClient() ya viven acá y no se reconstruyen en otro archivo.
// Devuelve arrays vacíos si R2 no está configurado: en modo disco local no hace falta permitir nada
// más que el propio origen del server, que server.js ya cubre con 'self'.
function getCspOrigins() {
  if (!isR2Enabled()) return { connectSrc: [], mediaSrc: [] };
  const presignOrigin = `https://${R2_BUCKET_NAME}.${R2_ACCOUNT_ID}.r2.cloudflarestorage.com`;
  const origins = { connectSrc: [presignOrigin], mediaSrc: [] };
  if (R2_PUBLIC_URL) {
    try {
      const publicOrigin = new URL(R2_PUBLIC_URL).origin;
      origins.mediaSrc.push(publicOrigin);
      origins.connectSrc.push(publicOrigin);
    } catch (_err) {
      // R2_PUBLIC_URL mal formada (ej. sin "https://"): no tiene sentido reventar el arranque del
      // server por esto acá — getPublicUrl() ya tira su propio error, más claro, apenas alguien
      // intente armar un link real con esta variable.
    }
  }
  return origins;
}
