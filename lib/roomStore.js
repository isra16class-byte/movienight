...
// Antes de este refactor (docs/PLAN-PANEL-ADMIN.md, sección 4), ROOM_TTL_HOURS se leía UNA sola vez
// acá arriba, al cargar el módulo — cambiarlo requería reiniciar el proceso. Ahora es una función que
// consulta lib/settings.js en cada llamada (barata: lee de una cache en memoria, ver ese archivo), lo
// que permite cambiarlo desde el panel de administración sin reiniciar nada. `null` es un valor válido
// (ver lib/settings.js, "nullable") que significa "nunca expira" — devuelve `null` tal cual para que el
// caller (acá abajo, y sweepExpiredRooms en server.js) decida cómo tratarlo, en vez de inventar un
// número mágico como Infinity que después haya que acordarse de manejar como caso especial en otro lado.
function getRoomTtlSeconds() {
  const hours = settings.getSetting('ROOM_TTL_HOURS');
...
function getClient() {
  if (!_client) {
    ...
    _client.on('error', (err) => {
      ...
      logger.error({ err }, 'Redis (roomStore) reportó un error de conexión');
    });
  }
  return _client;
}
...
async function closeConnection() {
  ...
  try {
    ...
    logger.error({ err }, 'Error cerrando la conexión a Redis (roomStore) durante el shutdown');
  }
}
...
function hydrateRoom(data) {
  return {
    ...
    viewers: 0,
    // emptySince (paso 7 del panel de administración, ver el comentario largo en makeRoom() de
    // server.js): no se persiste — igual que hostSocketId/userNames, depende de conexiones de socket
    // en vivo que no sobreviven a un reinicio. Se estampa "ahora" porque, igual que una sala recién
    // creada, arranca en 0 viewers hasta que alguien se una vía join-room.
    emptySince: Date.now(),
...
async function saveRoom(roomId, room) {
  ...
  try {
    ...
    if (ttlSeconds === null) {
      // "Nunca expira" (elección explícita desde el panel, ver getRoomTtlSeconds): un SET sin `EX` dejaría
      // la key con el TTL que ya tuviera de antes en Redis (o ninguno) en vez de garantizar que quede sin
      // vencimiento — PERSIST la deja sin TTL de forma explícita, sea cual sea su estado previo.
      multi.set(KEY_PREFIX + roomId, serializeRoom(room)).persist(KEY_PREFIX + roomId);
    } else {
      // TTL nativo de Redis (Fase 2.6): se refresca en cada guardado (`EX`, no `EXPIRE` aparte) —
      // mientras haya actividad real la key nunca llega a expirar; sin actividad y Redis la borra
      // sola, sin depender de que el proceso siga corriendo.
      multi.set(KEY_PREFIX + roomId, serializeRoom(room), 'EX', ttlSeconds);
    }
    await multi.sadd(ROOM_INDEX_KEY, roomId).exec();
  } catch (err) {
    logger.error({ err, roomId }, 'No se pudo guardar la sala en Redis');
  }
}
...
async function deleteRoom(roomId) {
  ...
  try {
    ...
    logger.error({ err, roomId }, 'No se pudo borrar la sala de Redis');
  }
}
...
async function loadAllRooms() {
  ...
  for (let i = 0; i < roomIds.length; i++) {
    ...
    try {
      ...
      logger.error({ err, roomId: roomIds[i] }, 'No se pudo leer la sala desde Redis (dato corrupto, se descarta)');
    }
  }
  return result;
}
