...
function makeRequireAdmin(db, log) {
  return async function requireAdmin(req, res, next) {
    ...
    try {
      ...
      // Nota (evaluado probando con Docker/Postgres real, ver docs/CHANGELOG.md): a propósito NO se
      // trata un error de conectividad (Postgres configurado pero caído en este momento) como 404
      // "no disponible" — ese código queda reservado para DATABASE_URL ausente (chequeo de arriba).
      // Postgres caído mid-request es una falla real que vale la pena que se note (log + 500), igual
      // que lib/db.js::ping() la reporta como `ok:false` en el healthcheck en vez de disfrazarla de
      // "función no configurada". Clasificar por err.code (ECONNREFUSED/ENOTFOUND/etc.) para devolver
      // 404 en ese caso se probó y se descartó: no hay una lista cerrada de códigos que cubra todo
      // "es conectividad", y el día que un bug real dispare por accidente uno de esos códigos, un
      // admin vería "No disponible" en vez de un 500 que lo hiciera sospechar que algo se rompió.
