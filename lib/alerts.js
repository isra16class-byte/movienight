...
// Cuántos chequeos seguidos en falla hacen falta antes de mandar la primera alerta. Con el intervalo
// por default de 1 minuto (ver ALERT_CHECK_INTERVAL_MS en server.js) y este default, hacen falta 3
// minutos de caída sostenida antes de la primera alerta — suficiente para no disparar por un blip
// de red de un segundo, pero sin tardar tanto que alguien se entere tarde de una caída real.
const parsedFailureThreshold = parseInt(process.env.ALERT_HEALTH_FAILURE_THRESHOLD, 10);
const HEALTH_FAILURE_THRESHOLD = (Number.isFinite(parsedFailureThreshold) && parsedFailureThreshold > 0)
  ? parsedFailureThreshold
  : 3;

// Una vez que ya se mandó una alerta (de cualquiera de los dos tipos) y el problema sigue, no tiene
// sentido mandar un email nuevo en cada chequeo (cada 1 minuto por default) mientras dure la caída —
// eso satura la casilla sin agregar información nueva. Este cooldown espacia los reintentos de
// "sigue caído" mientras el problema persista. Default 30 minutos.
const parsedCooldownMs = parseInt(process.env.ALERT_COOLDOWN_MS, 10);
const ALERT_COOLDOWN_MS = (Number.isFinite(parsedCooldownMs) && parsedCooldownMs > 0)
  ? parsedCooldownMs
  : 30 * 60 * 1000;
...
async function checkHealthAndAlert(healthy, checks) {
  ...
  if (healthAlertActive && (now - lastHealthAlertAt) < ALERT_COOLDOWN_MS) return;
  ...
  await sendAlert(
    ...
    `(umbral configurado: ${HEALTH_FAILURE_THRESHOLD}).\n` +
...
async function checkR2ErrorsAndAlert(r2Snapshot) {
  ...
  const now = Date.now();
  if ((now - lastR2AlertAt) < ALERT_COOLDOWN_MS) {
